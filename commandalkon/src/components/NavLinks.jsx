    ...
    {links.map((link) => (
        <div>
          <div className="px-3 text-left md:cursor-pointer group">
            <h1
              className="py-2 text-black sm:text-gray-300 hover:text-red-500 rounded-md duration-500 text-lg sm:text-xs font-semibold flex justify-between items-center md:pr-0 pr-5 group"
              onClick={() => {
                heading !== link.name ? setHeading(link.name) : setHeading("");
                setSubHeading("Inbound Settings");
              }}
            >
              {link.name}
